...
import { KonvaData, SlideshowSlide } from "../types";
...
interface KonvaEditorReturn {
  ...
  saveChanges: () => Promise<SlideshowSlide | null>;
  undo: () => void;
  redo: () => void;
  canUndo: boolean;
  canRedo: boolean;
...
export function useKonvaEditor(slideId: number): KonvaEditorReturn {
  ...
  const currentSlide = isValidSlideId
    ? currentSlideshow?.slides.find((slide) => slide.id === slideId)
    : undefined;
  ...
  const saveChanges = useCallback(async () => {
    ...
    try {
      ...
      // Mettre à jour la slide en utilisant directement l'API
      const result = await slideshowApi.updateSlide(currentSlide.id, {
        konvaData,
      });
  ...
  }, [currentSlide, elements, isSaving, isValidSlideId]);
